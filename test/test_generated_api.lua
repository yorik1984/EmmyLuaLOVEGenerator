    ...
    for _, fileData in ipairs(testFiles) do
        ...
        if not testFileExists(testFile) then
            ...
            if annotations.meta and annotations.hasParams and annotations.hasReturns then
                ...
                if not annotations.meta then print("     - Missing @meta") end
                if not annotations.hasParams then print("     - Missing @param") end
                if not annotations.hasReturns then print("     - Missing @return") end
                failed = failed + 1
            end
